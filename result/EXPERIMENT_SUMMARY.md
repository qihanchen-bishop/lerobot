...
`eval/` 下目前有 **949 条有效评估记录**，19 份 `eval_results.jsonl` 与对应 episode 文件数量完全一致。另外有 2 个已建立但从未产生评估数据的实验目录：`1B` 和 `2A`。

按完整的 3 个任务、3x4 网格协议计算：

- **7 组完整实验，共 792 条记录。**
- **12 组部分测试或中途停止实验，共 157 条记录。**
- **2 组未开始实验，0 条记录。**

当前最可靠的总体结论：

- ACT 是唯一在三代评估中都稳定完成完整协议的策略，也是目前最可靠的基线。
- 旧 setup 的大样本 ACT 为 `273/360 (75.8%)`；`eval20` 的 ACT 为 `58/72 (80.6%)`；newsetup ACT 为 `48/72 (66.7%)`。
- `eval20` 中 ACT、SEM-1、DP、SEM-2 依次为 `80.6%`、`72.2%`、`68.1%`、`62.5%`。
- newsetup 中 ACT 为 `66.7%`，DP 为 `54.2%`。两者都低于旧 setup，说明 newsetup 的数据、视角或任务分布没有自然带来性能提升。
- newsetup 最新相机编码实验中，`ACT-CE-GATED` 已完成 cube 两轮及 paperball/screw 各一轮，共 `38/48 (79.2%)`；相同48次 ACT 为 `34/48 (70.8%)`。增益主要来自 cube 的 `21/24 (87.5%)`，但实验尚未完成完整72次协议。
- `ACT-CE-ZERO` 已完成 cube 两轮和 screw 一轮，共 `25/36 (69.4%)`，与相同36次 ACT 完全持平；原始 `ACT-CE` cube 第一轮仅 `6/12 (50.0%)`。
- 对 Gated checkpoint 的真实帧消融显示，最终 camera embedding 通路对动作几乎无直接影响；当前较好的实测性能来自一套不同的共享 ACT 权重，尚不能归因于推理时使用了相机身份编码。
...
| `newsetup` | 2026-08-24 至 08-26 | `bettersetup` 重采数据并重训 | 每任务 12 格，每格 2 次；完整协议 72 次 | 比较 ACT、DP、语义策略及相机/视角编码变体 |
...
|---|---:|---|---:|---|---|
| `object3color2/act` | 360/360 | 3 任务，12 格，每格 10 次 | 273/360 (75.8%) | **完成** | 保留为最可靠旧基线 |
...
| `newsetup/act/ACT` | 72/72 | 3 任务，12 格，每格 2 次 | 48/72 (66.7%) | **完成** | newsetup 主基线 |
| `newsetup/act/ACT-CE` | 12/72 | cube，12 格，每格 1 次 | 6/12 (50.0%) | 部分完成 | 默认随机 camera embedding 表现较差 |
| `newsetup/act/ACT-CE-GATED` | 48/72 | cube 每格 2 次；paperball/screw 每格 1 次 | 38/48 (79.2%) | **部分完成，当前最有潜力** | 补齐 paperball/screw 第 2 次 |
| `newsetup/act/ACT-CE-ZERO` | 36/72 | cube 每格 2 次；screw 每格 1 次 | 25/36 (69.4%) | 部分完成 | 补齐 screw 第 2 次和 paperball |
...
| `newsetup/mask_act/VIEWFUS-V1` | 11/72 | cube，除 `(0,0)` 外 11 格各 1 次 | 5/11 (45.5%) | 部分完成 | 初步弱于匹配 ACT，不优先补齐 |
...
|---|---:|---:|---:|---:|
| `object3color2/act` | 100/120 (83.3%) | 84/120 (70.0%) | 89/120 (74.2%) | 273/360 (75.8%) |
...
| `newsetup/act` | 17/24 (70.8%) | 16/24 (66.7%) | 15/24 (62.5%) | 48/72 (66.7%) |
...
- newsetup 中 ACT 比 DP 总体高 12.5 pp，其中 cube 高 20.8 pp、screw 高 16.7 pp；paperball 完全相同。
- newsetup ACT 的 screw 从旧 `eval20` 的 91.7% 降到 62.5%，这是当前最需要复核的数据/setup 变化。

### 4.3 阶段到达率

| 实验 | 曾暴露物体 | 与遮挡物分离 | 最终成功 |
|---|---:|---:|---:|
| `object3color2/act` | 353/360 (98.1%) | 323/360 (89.7%) | 273/360 (75.8%) |
...
| `newsetup/act` | 67/72 (93.1%) | 64/72 (88.9%) | 48/72 (66.7%) |
...
| `newsetup/act` | 13/24 (54.2%) | 15/24 (62.5%) | 20/24 (83.3%) |
...
### 5.6 newsetup VIEWFUS-V1

VIEWFUS-V1 在 cube 上测试了 11 个网格，每格 1 次，缺少 `(0,0)`，结果为 `5/11 (45.5%)`；相同 11 个网格和试次的 ACT 为 `8/11 (72.7%)`。

- 最终暴露、分离、到达区域、成功依次为 `8/11`、`8/11`、`6/11`、`5/11`。
- `y=0` 为 `0/3`，`y=1` 为 `2/4`，`y=2` 为 `3/4`，仍然明显受空间位置影响。
- 当前数据不支持 ViewFus 的视角/模态编码优于 ACT。由于还缺一个网格且只有单轮，应标记为负向初筛，而不是最终 0/72 式结论。

### 5.7 newsetup ACT 相机编码实验

三种相机编码变体都保持 ACT 的 `front RGB + side RGB + state` 输入、60 步 action chunk、同步执行和无历史融合，主要差异是 camera identity embedding 的初始化与缩放方式。

#### 当前覆盖和任务结果

| 策略 | cube | paperball | screw | 当前全部记录 |
|---|---:|---:|---:|---:|
| ACT | 17/24 (70.8%) | 16/24 (66.7%) | 15/24 (62.5%) | 48/72 (66.7%) |
| ACT-CE | 6/12 (50.0%) | 未测试 | 未测试 | 6/12 (50.0%) |
| ACT-CE-GATED | **21/24 (87.5%)** | **10/12 (83.3%)** | 7/12 (58.3%) | **38/48 (79.2%)** |
| ACT-CE-ZERO | 18/24 (75.0%) | 未测试 | 7/12 (58.3%) | 25/36 (69.4%) |

由于 paperball 和 screw 尚未完成第 2 次，不能把各策略“当前全部记录”的成功率当作完整协议排名。公平配对结果为：

| 比较 | 变体 | 相同试次 ACT | 差异 |
|---|---:|---:|---:|
| ACT-CE，cube 第 1 次 | 6/12 (50.0%) | 9/12 (75.0%) | -25.0 pp |
| ACT-CE-GATED，当前48次 | **38/48 (79.2%)** | 34/48 (70.8%) | **+8.3 pp** |
| ACT-CE-ZERO，当前36次 | 25/36 (69.4%) | 25/36 (69.4%) | 0.0 pp |

Gated 的主要增益来自 cube：两轮分别为 `10/12` 和 `11/12`，ACT 为 `9/12` 和 `8/12`。Gated 在 cube 中到达目标区域 21 次，并且 21 次全部最终成功；ACT 到达区域 20 次，最终成功 17 次，说明差异主要出现在目标附近的持续推进和最终状态保持。

空间上，Gated 在当前48个匹配试次的 `y=1` 为 `14/16 (87.5%)`，ACT 为 `10/16 (62.5%)`；`y=0` 和 `y=2` 两者分别同为 `9/16` 和 `15/16`。其固定弱点是 cube `(3,0)`，两轮均失败。Zero 的主要弱点则是左侧 `x=0`：在 cube+screw 的匹配36次中为 `5/9`，ACT 为 `8/9`。

#### Gated 机制消融

Gated checkpoint 的标量 gate 最终为 `-2.626e-5`，camera embedding 权重 RMS 约 `0.01915`，因此有效 camera embedding RMS 只有约 `5e-7`。使用全部48条 Gated 轨迹的中间帧测得：

| 激活或输出 | 实测尺度 |
|---|---:|
| front 投影后 RGB feature RMS | 0.482 |
| side 投影后 RGB feature RMS | 0.554 |
| 有效 camera embedding / RGB feature RMS | 约 `1e-6` |
| 第一层 LayerNorm 后相对扰动 | `1.94e-6` |
| 最后 Encoder 层相对扰动 | `1.22e-6` |
| Decoder 输出相对扰动 | `3.55e-7` |
| 打开/关闭 gate 的关节动作 RMS 差异 | `8.73e-6` 度 |
| 打开/关闭 gate 的最大关节动作差异 | `6.22e-5` 度 |

所以，最终 checkpoint 在推理时打开或关闭 camera embedding，动作基本完全相同。但 Gated 关闭 gate 后与基础 ACT 的动作差异 RMS 仍为 `1.72` 度，最大差异为 `11.75` 度；两者共享权重的整体相对 RMS 差异约为 3.2%。

因此当前应采用以下表述：

- **Gated checkpoint 确实学到了一套与 ACT 不同、当前实测更好的共享策略参数。**
- **最终 camera embedding 直接通路在推理时几乎没有贡献。**
- 不能排除 gate 在训练中间阶段曾影响共享权重，但当前没有 gate 曲线或中间 checkpoint 可以验证。
- 现有成功率支持继续补齐 Gated 实验，但不能据此声称“相机身份编码已经产生了因果收益”。
...
| ACT-CE | ACT + 每个视角的 camera identity embedding | 默认 Embedding 初始化尺度过大，cube 初筛弱于 ACT |
| ACT-CE-GATED | ACT-CE embedding 乘可学习标量 gate | 当前 checkpoint 表现最好，但最终 gate 通路近似关闭 |
| ACT-CE-ZERO | camera embedding 从零初始化并直接相加 | 学到小幅非零视角差异，当前成功率与 ACT 持平 |
...
| VIEWFUS-V1 | RGB/语义视角特征加入视角与模态编码后融合 | cube 仅完成 11 次，初步弱于匹配 ACT |
...
所有主实验均为 30 FPS、3x4 网格、双臂、front+side、无历史 action fusion。除首条旧 ACT 使用 `latest_nonblocking` 外，其余 948 条均使用 `wait_new_frame`。

| 策略族 | 模型预测长度 | 每次执行/重规划 | 执行方式 | AMP | 其他 |
|---|---:|---:|---|---|---|
| ACT/ACT-CE/GATED/ZERO | 60 | 60 | 同步 | 关 | 一次预测基本执行完整 chunk；CE 变体只改变特征编码 |
...
- 全部 949 条记录中只有 1 条人工最终结果与自动最终判定不一致，整体一致率约 99.9%。最终成功率仍以人工 `result` 为主。
...
6. ACT-CE-GATED 是 newsetup 当前最有潜力的新变体，特别是 cube；但最终 camera embedding 通路近似关闭，应该把它视为“更好的 Gated 训练 checkpoint”，而不是已经证明有效的相机编码机制。
7. ACT-CE-ZERO 当前与 ACT 持平；默认 ACT-CE 和 VIEWFUS-V1 的初步结果均弱于匹配 ACT。
...
5. 补齐 ACT-CE-GATED 的 paperball/screw 第 2 次；这是判断其总体优势是否成立的最高优先级实验。
6. 补齐 ACT-CE-ZERO 的 screw 第 2 次和 paperball，同时重点复测 `x=0`，确认左侧退化是否稳定存在。
7. 后续 Gated 训练记录 gate、有效 embedding RMS、embedding/RGB feature RMS、开关 gate 的动作差异，并保留中间 checkpoint；至少使用多个相同 seed 的 ACT/Gated 配对训练区分机制收益与训练方差。
...
5. 默认 ACT-CE 和 VIEWFUS-V1 当前不应直接补满完整协议；若继续，应先修正 embedding 尺度或完成离线消融。

## 11. 相关报告

- `result/act/REPORT.md`：`object3color2/act` 的 360 次大样本详细报告和图表。
- `eval/newsetup/REPORT.md`：newsetup 的阶段表现、失败模式和位置分析。
- `eval/newsetup/act/ACT/ACT_FAILURE_AND_SPECIAL_CASES.md`：newsetup ACT 的24条失败视频、空间热区和5条特殊成功记录。
