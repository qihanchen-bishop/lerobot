...
% \subsubsection{关系信息与执行更新}
\subsubsection{Relational Information and Execution Updates}
% 完整历史覆盖可因小幅动作偏差失效。
...
% 因而，较小覆盖系数是否收紧上界，还取决于同一表示下的条件误差差异；附加RGB的确定性语义图并不自动改善覆盖。
Thus, whether a smaller coverage factor tightens the bound also depends on the conditional-error discrepancy.
Appending a deterministic semantic map to retained RGB does not by itself improve coverage.
% D_phi是固定策略的条件动作误差差异，而非任务信息损失；C、D分别讨论信息保留与执行利用，不以本节覆盖条件为前提。
Here, $D_\phi$ measures conditional action-error differences for a fixed policy, not task-information loss.
Sections~\ref{sec:representationanalysis} and~\ref{sec:feedbackanalysis} next examine task-relevant information retention and the use of new execution evidence through action consequences, without requiring the coverage conditions above.
