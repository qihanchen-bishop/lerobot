\subsection{From Imitation Error to Task Success}
\noindent
...
由性能差分恒等式\cite{schulman2015}的有限时域形式，
\begin{equation}
 F(\pi)-F(\pi_E)
 =\sum_{t=0}^{T-1}\E_{d_\pi^t} A_t^E(x,\pi(x)).
 \label{eq:pd}
\end{equation}
下面给出该恒等式及风险界的证明。等式的对象是\emph{闭环访问分布下、按任务后果衡量的误差}，
不是训练集的平均动作距离。

\subsubsection{覆盖、泛化与任务敏感性的共同作用}
记专家分布平方动作风险为
...
部署分布偏移和误差传播是模仿学习的重要困难\cite{ross2011}；稳定性约束下的样本复杂度分析\cite{tu2022}提供相关背景，但不直接保证本方法满足以下条件。
...
这一有限类结果不能以实验组数替代神经网络假设类复杂度，因此不作为本文系统的数值证书。
已有训练log的L1加KL总和不等于这里的平方动作风险。
如需比较，须额外在同一未见episode、相同目标槽位与归一化下重算指标。


\subsubsection{更小误差不等于更高成功率}
