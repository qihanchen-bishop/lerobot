\subsection{History-Conditioned Feedback for Execution Correction}
\noindent
...
\subsubsection{为何频繁ACT与时间集成可能效果不好}
...
实际比较应固定查询频率、预测年龄及时间集成权重，并独立测量端到端延迟。
式\eqref{eq:stale}描述一种可能的失效机制，但不单独识别实机失败的原因。
\draftnote{待补：实际部署的时间集成权重、查询频率与结果年龄，结合受控消融判断该机制是否成立。}

\subsubsection{残差的区别与任务改进边界}
