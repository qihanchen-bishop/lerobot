...
% 反馈表示是确定目标槽位候选指令所需计划信息、观测、执行历史和运行记录的因果表示。
Let $Z_t^{\mathrm{fb}}$ be a causal representation of the planning information, observations, execution history, and runtime records needed to determine the target-slot candidate command,
% 生成信息 $\mathcal U=\sigma(Z_t^{\mathrm{fb}})\subseteq\mathcal F_h$。
generating $\mathcal U=\sigma(Z_t^{\mathrm{fb}})\subseteq\mathcal F_h$.
% 沿用C的完整读出前表示约定，候选选择及后处理由该表示可测地决定。
As in Section~\ref{sec:representationanalysis}, this is the complete pre-readout representation: candidate selection and postprocessing are measurable functions of it.
% tau_p为有效计划所用观测时刻，tau_o为候选所用最新观测时刻，t为目标执行时刻。
For a valid plan, let $\tau_p$ be the time of the observation used by that plan, $\tau_o$ the latest observation time used by the candidate, and $t$ the target execution time, with $\tau_p\le\tau_o\le t$.
% 编码与观测延迟可能限制保留的决策信息。
Encoding and observation delay may limit the decision-relevant information retained.
...
% 参照与修正控制器在共同过程假设下共享计算状态转移律（含接收过程），仅执行修正不同；不要求不同闭环轨迹有相同实际完成时刻。
The reference and corrected controllers share the same computation-state transition law under Assumption~\ref{ass:commonprocess}, including the reception process, and differ only in the executed correction.
...
% 计划时刻的关系估计精度不能替代目标执行时刻的关系估计精度。
Relation-estimation accuracy at the planning time $\tau_p$ cannot substitute for accuracy at the target execution time $t$ (Appendix~\ref{app:feedbackrelationtime}).
...
% 该分析性充分条件不能仅凭模仿与辅助训练损失认证。
This analytical sufficient condition cannot be certified from imitation and auxiliary training losses alone.
% 未采用惩罚是保守的：被拒绝的候选未必有益；在r=0约定下，无候选槽位相对于当前候选的局部采用损失为零，仍可计入未采用惩罚。
The nonadoption penalty is conservative: rejected corrections need not be beneficial, and under the $r=0$ convention a slot with no candidate has zero local adoption loss relative to its defined candidate, yet may still contribute to $p_{\mathrm{miss}}$.
