...
% 前述分析讨论决策所需信息的保留。执行中，新观测与动作--响应历史还可能揭示当前观测无法区分的情形。
The preceding analysis concerns the preservation of information needed for decisions.
During execution, new observations and action--response history may reveal distinctions unresolved by the current observation.
% 下面分析这些新增信息的决策价值，以及它们通过动作修正产生任务收益的条件。
We analyze the decision value of this additional information and the conditions under which it yields task benefits through action corrections.
