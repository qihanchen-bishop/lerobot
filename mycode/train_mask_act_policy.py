...
class MaskQualityStore:
    """In-memory frame-level validity generated by sam2_mask_quality.py."""
...
class MaskACTPolicy(nn.Module):
    ...
    def semantic_segmentation_loss(
        ...
        class_validity_by_view: list[Tensor] | None = None,
    ) -> tuple[Tensor, Tensor, Tensor, dict[str, float]]:
        ce_losses = []
        dice_by_class = []
        dice_valid_by_class = []
        if class_validity_by_view is None:
            class_validity_by_view = [None] * len(logits_by_view)
        for logits, target, foreground_validity in zip(
            logits_by_view,
            targets_by_view,
            class_validity_by_view,
            strict=True,
        ):
            if foreground_validity is None:
                ce_losses.append(F.cross_entropy(logits, target, weight=self.semantic_class_weights))
                sample_class_validity = torch.ones(
                    logits.shape[:2],
                    dtype=torch.bool,
                    device=logits.device,
                )
            else:
                foreground_validity = foreground_validity.to(device=logits.device, dtype=torch.bool)
                if foreground_validity.shape != (logits.shape[0], logits.shape[1] - 1):
                    raise ValueError(
                        "Mask quality class validity must have shape "
                        f"{(logits.shape[0], logits.shape[1] - 1)}, got {tuple(foreground_validity.shape)}."
                    )
                background_validity = foreground_validity.all(dim=1, keepdim=True)
                sample_class_validity = torch.cat([background_validity, foreground_validity], dim=1)
                pixel_validity = sample_class_validity.gather(1, target.flatten(1)).reshape_as(target)
                ...
                ce_losses.append(
                    (ce_per_pixel * pixel_validity).sum()
                    / (pixel_class_weight * pixel_validity).sum().clamp_min(1e-6)
                )
            ...
            dims = (0, 2, 3)
            dice_weights = sample_class_validity.to(dtype=logits.dtype).unsqueeze(-1).unsqueeze(-1)
            ...
            dice_valid_by_class.append(sample_class_validity.any(dim=0))
    ...
    def forward(self, batch: dict[str, Tensor], raw_batch: dict[str, Tensor]) -> tuple[Tensor, dict[str, float]]:
        ...
        if self.uses_semantic_maps():
            ...
            class_validity_by_view = None
            quality_logs = {}
            if quality_validity is not None:
                quality_validity = quality_validity.to(device=device, dtype=torch.bool)
                class_validity_by_view = [quality_validity[:, view_idx] for view_idx in range(len(self.rgb_keys))]
                quality_logs = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Offline SAM2 quality directory from sam2_mask_quality.py; unreliable labels are masked from losses.",
    )
    parser.add_argument(
        "--mask-quality-min-score",
        type=float,
        default=0.60,
        help="Additional minimum offline label-quality score required by training.",
    )
    parser.add_argument(
        "--phase-labels",
        type=Path,
